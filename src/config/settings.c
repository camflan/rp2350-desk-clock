...
#define SETTINGS_FLASH_ADDR   ((const uint8_t *)(XIP_BASE + SETTINGS_FLASH_OFFSET))

static settings_t current;

/* CRC32 with standard polynomial 0xEDB88320 (table-based) */
static const uint32_t crc32_table[256] = {
    ...
    0x35B5A8FA, 0x42B2986C, 0xDBBBB9D9, 0xACBCB9AF,
    ...
    0x44042D73, 0x33031DE5, 0xAA0A4C5F, 0xDD0D7B29,
    0x4D6F0E90, 0x3A6E1E06, 0xA3671BE5, 0xD4602273,
    ...
    0xDF60EFC3, 0xA8670955, 0x31684D8E, 0x4669BE79,
    ...
    0x86D3D2D4, 0xF1D4E242, 0x68DDB3F6, 0x1FDA836E,
...
void settings_load(void) {
    settings_t stored;
    memcpy(&stored, SETTINGS_FLASH_ADDR, sizeof(stored));
    ...
    /* Validate CRC over everything except the crc32 field itself */
    uint32_t expected = calc_crc32((const uint8_t *)&stored,
                                   offsetof(settings_t, crc32));
    if (stored.crc32 != expected) {
        settings_defaults();
        return;
    }
...
void settings_save(void) {
    current.crc32 = calc_crc32((const uint8_t *)&current,
                                offsetof(settings_t, crc32));

    /* Pad to FLASH_PAGE_SIZE for programming */
    uint8_t page[FLASH_PAGE_SIZE];
    memset(page, 0xFF, sizeof(page));
    memcpy(page, &current, sizeof(current));
